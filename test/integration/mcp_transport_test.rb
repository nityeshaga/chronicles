...
class McpTransportTest < ActionDispatch::IntegrationTest
  ...
  test "a valid session can list the registered tools" do
    ...
    expected = %w[ list_posts get_post list_tags create_post update_post publish_post unpublish_post delete_post update_tag upload_image create_embed create_html_page update_html_page create_html_card update_html_card create_explorable update_explorable list_notes resolve_note list_ships create_ship update_ship publish_ship upload_ship_media ]
