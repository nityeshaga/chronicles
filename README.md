...
**Editor (Lexxy).** Its content styles are `:where(.lexxy-content)`-scoped at zero specificity — *designed* to be overridden by the app. WYSIWYG parity works by factoring the published `.gh-content` typography into selector lists **shared** with `.editor-canvas__body .lexxy-editor__content` (two classes deep, so they outrank the gem on specificity): change one, you change both, which is the point. Both layouts load the same reader fonts through `layouts/_fonts`. Theme the chrome via `--lexxy-*` CSS variables, in the one token block on `.editor-canvas__body lexxy-editor`; the highlight swatches are `--highlight-*` on `.gh-content, lexxy-editor` (not `:root`, where Lexxy defines its own — a nearer ancestor wins whatever the stylesheet order) because the stored `<mark style>` reads them on both surfaces. Never fight the gem with `!important`. Note the site's `html{font-size:62.5%}`: naked `rem` values from third-party CSS compute smaller than you expect — `--lexxy-content-margin` is zeroed for that reason and the shared rules own the block rhythm. Readers never download Lexxy: the public layout's entry is `application.js`, the writing layout's is `writing` (`app/javascript/writing/index.js`), and only the latter imports the editor. A controller only the author's pages wear goes in `app/javascript/writing/`, not `controllers/` — `eagerLoadControllersFrom` ships everything under `controllers/` to every public page. **Lexxy extensions register once.** `configure()` deep-merges with a spread, which *replaces* arrays: two modules each calling `configure({ global: { extensions: [...] } })` leave only the last one's extensions. Extension files (`lexxy_embed_frames.js`, `lexxy_body_headings.js`, `lexxy_inline_code.js`, `lexxy_html_cards.js`, `lexxy_code_blocks.js`, `lexxy_heading_hotkeys.js`) export their class; `app/javascript/writing/index.js` makes the single `configure` call with all of them. **The toolbar has to be a Lexxy element before the body goes in.** `<lexxy-toolbar>` is a child of `<lexxy-editor>`, and a Turbo visit inserts the new body as one subtree — custom elements upgrade parent before child there, so the editor's `connectedCallback` calls `setEditor` on an element that isn't a toolbar yet, throws, and leaves Lexical unmounted: a canvas that takes keystrokes and drops them while the status still reads "Saved". A full page load never shows it, because Lexxy defines `lexxy-toolbar` before `lexxy-editor` and definition order is upgrade order for elements already in the document. The `turbo:before-render` listener in `writing/index.js` adopts the incoming body and upgrades its toolbars first; the adoption is load-bearing, because Turbo parses the response with `DOMParser` and `customElements.upgrade` is a no-op on a document with no browsing context. **Three things the gem will not tell you.** `lexxy:attributes-change` is dispatched only by the Hotwire Native adapter, so a browser never fires it — read the caret from the document's own `selectionchange`, not from that event. A toolbar button activated with `element.click()` arrives as a `PointerEvent` with `pointerId: -1`, which is how Chrome reports a keyboard press and how Lexxy tells the two apart: it then tags the update `skip-dom-selection` and the DOM selection collapses under the reconcile, so anything proxying a click to the toolbar must dispatch a `PointerEvent` with a real `pointerId`. And `lexxy-link-dropdown` binds its Unlink button when the toolbar connects, not when the panel opens, so unlinking is one click on that button with the panel still shut — no extension, no open-and-close. **Three things the gem will not tell you.** `lexxy:attributes-change` is dispatched only by the Hotwire Native adapter, so a browser never fires it — read the caret from the document's own `selectionchange`, not from that event. A toolbar button activated with `element.click()` arrives as a `PointerEvent` with `pointerId: -1`, which is how Chrome reports a keyboard press and how Lexxy tells the two apart: it then tags the update `skip-dom-selection` and the DOM selection collapses under the reconcile, so anything proxying a click to the toolbar must dispatch a `PointerEvent` with a real `pointerId`. And `lexxy-link-dropdown` binds its Unlink button when the toolbar connects, not when the panel opens, so unlinking is one click on that button with the panel still shut — no extension, no open-and-close. **Blocks are the toolbar's to make.** Anything that inserts a block — the slash menu today — names a toolbar button and clicks it, rather than inserting markup: Lexxy's own `<lexxy-prompt>` inserts HTML *at the caret*, so a quote or a heading typed into a paragraph lands nested inside it (`<p><blockquote>…</blockquote></p>`). A prompt is right for inline things (`@`-mentions), wrong for blocks. Clear the trigger text through Lexical (`$getSelection()` → top-level element → `clear()`), never through the DOM, and let the command transform the empty block. **Keys the editor must not see are caught in the capture phase** on `.editor-canvas__body`: Lexical's own listener sits on the contenteditable below it, so a capture listener on the ancestor answers arrows and Enter first — verified, not assumed. **`lexxy:change` only fires when the body's HTML changed**, so it can't see a caret moving; pair it with `selectionchange` when you need to know the caret left. **The editor is dead on a Turbo visit** (as of Lexxy 0.9.31): a visit inserts `<lexxy-editor>` with its `<lexxy-toolbar>` child in one subtree, custom elements upgrade parents before children, and the editor throws `toolbarElement.setEditor is not a function` attaching a toolbar that hasn't upgraded — the canvas comes up blank and every keystroke is discarded (the form carries no `post[body]`, so nothing is overwritten). A full page load has the parser build the tree before Lexxy defines either element, so they upgrade in registration order and it works. Real-browser tests must `visit` the editor, not click into it. **The toolbar has to be a Lexxy element before the body goes in.** `<lexxy-toolbar>` is a child of `<lexxy-editor>`, and a Turbo visit inserts the new body as one subtree — custom elements upgrade parent before child there, so the editor's `connectedCallback` calls `setEditor` on an element that isn't a toolbar yet, throws, and leaves Lexical unmounted: a canvas that takes keystrokes and drops them while the status still reads "Saved". A full page load never shows it, because Lexxy defines `lexxy-toolbar` before `lexxy-editor` and definition order is upgrade order for elements already in the document. The `turbo:before-render` listener in `writing/index.js` adopts the incoming body and upgrades its toolbars first; the adoption is load-bearing, because Turbo parses the response with `DOMParser` and `customElements.upgrade` is a no-op on a document with no browsing context. **Three things the gem will not tell you.** `lexxy:attributes-change` is dispatched only by the Hotwire Native adapter, so a browser never fires it — read the caret from the document's own `selectionchange`, not from that event. A toolbar button activated with `element.click()` arrives as a `PointerEvent` with `pointerId: -1`, which is how Chrome reports a keyboard press and how Lexxy tells the two apart: it then tags the update `skip-dom-selection` and the DOM selection collapses under the reconcile, so anything proxying a click to the toolbar must dispatch a `PointerEvent` with a real `pointerId`. And `lexxy-link-dropdown` binds its Unlink button when the toolbar connects, not when the panel opens, so unlinking is one click on that button with the panel still shut — no extension, no open-and-close. **Blocks are the toolbar's to make.** Anything that inserts a block — the slash menu today — names a toolbar button and clicks it, rather than inserting markup: Lexxy's own `<lexxy-prompt>` inserts HTML *at the caret*, so a quote or a heading typed into a paragraph lands nested inside it (`<p><blockquote>…</blockquote></p>`). A prompt is right for inline things (`@`-mentions), wrong for blocks. Clear the trigger text through Lexical (`$getSelection()` → top-level element → `clear()`), never through the DOM, and let the command transform the empty block. **Keys the editor must not see are caught in the capture phase** on `.editor-canvas__body`: Lexical's own listener sits on the contenteditable below it, so a capture listener on the ancestor answers arrows and Enter first — verified, not assumed. **`lexxy:change` only fires when the body's HTML changed**, so it can't see a caret moving; pair it with `selectionchange` when you need to know the caret left. **The editor is dead on a Turbo visit** (as of Lexxy 0.9.31): a visit inserts `<lexxy-editor>` with its `<lexxy-toolbar>` child in one subtree, custom elements upgrade parents before children, and the editor throws `toolbarElement.setEditor is not a function` attaching a toolbar that hasn't upgraded — the canvas comes up blank and every keystroke is discarded (the form carries no `post[body]`, so nothing is overwritten). A full page load has the parser build the tree before Lexxy defines either element, so they upgrade in registration order and it works. Real-browser tests must `visit` the editor, not click into it. **Three things the gem will not tell you.** `lexxy:attributes-change` is dispatched only by the Hotwire Native adapter, so a browser never fires it — read the caret from the document's own `selectionchange`, not from that event. A toolbar button activated with `element.click()` arrives as a `PointerEvent` with `pointerId: -1`, which is how Chrome reports a keyboard press and how Lexxy tells the two apart: it then tags the update `skip-dom-selection` and the DOM selection collapses under the reconcile, so anything proxying a click to the toolbar must dispatch a `PointerEvent` with a real `pointerId`. And `lexxy-link-dropdown` binds its Unlink button when the toolbar connects, not when the panel opens, so unlinking is one click on that button with the panel still shut — no extension, no open-and-close. **Lexxy is running before any of these controllers exist.** That entry imports Lexxy — which upgrades `<lexxy-editor>` and fires `lexxy:initialize` — *before* it calls `eagerLoadControllersFrom("writing", …)`, so a `lexxy:initialize->…` action never fires: nothing is listening yet. Do load-time work in `connect()`, where the editor is already built and there is prose to read. **And never paint into `.lexxy-editor__content`.** Lexical reconciles every node in there against its own model, so a marker or a highlight added by hand is one it will either throw away on the next update or take into the document and save. Overlays — the TK gutter is the worked example — go in an absolutely-positioned sibling that measures the prose with `Range.getClientRects()` and writes nothing to it. One consequence worth knowing: the editor bar's word count and `reading_time` are not the same number to the word, because Ruby's `strip_tags` glues one block onto the next ("…start.Every night…") where the browser's `innerText` breaks a line — on the longest imported post that is 42,291 against 40,308. The bar recounts at connect rather than at the first keystroke so the correction lands while the page is still arriving.

**Keyboard.** A toolbar button's `data-hotkey` is matched against `event.key`, and on a Mac keyboard Option turns the character into a symbol — ⌥2 arrives as `™` — so **no Option chord can ride `data-hotkey`**. The heading chords (⌘⌥2/3/4, ⌘⌥0) are an extension listening on the editor root and matching `event.code` instead, which names the physical key whatever Option did to it — dispatching Lexxy's named `applyHeadingFormat` command with the tag, never one of its private preset commands. WebDriver's synthetic `Alt+2` reports the plain digit, so a passing browser test proves less than it looks like — `test/system/editor/hotkeys_test.rb` also dispatches the real shape (`key: "™"`, `code: "Digit2"`) by hand. Exclude Shift from a chord like this (Lexxy and Stimulus both compare all four modifiers exactly), and turn away `getModifierState("AltGraph")` unless Cmd is held, or `AltGr+2` stops typing `²` on a European layout. Everything else is a Stimulus action on the form or the shell, except ⌘/: Stimulus 3.2 can't filter `/`, so that one reads `event.key` in the controller. A key that opens a page (⌘P) reads the href off the server-stamped link and `window.open`s it — *clicking* the link would navigate the editor away, because on a live post that button has no `target`. The shortcut sheet is a `<dialog>` in ERB, and `test/integration/shortcut_sheet_test.rb` asserts it prints every hotkey the toolbar declares.

**Overlays and focus.** Both editor overlays hide behind `visibility: hidden`, and a hidden element refuses focus — so transitioning `visibility` in both directions left a panel un-focusable for the first frames of its opening, and `focus()` on its first control silently did nothing. The transition delays visibility only on the way *closed* (the direction it was for), so a panel can take focus in the same frame the class lands. And note the site's `* { margin: 0 }` reset: it takes a modal `<dialog>`'s UA centring with it, so `margin: auto` has to be put back by hand. A modal is modal: keys that open a panel stand down while the shortcut sheet is up, or the panel lands behind its backdrop where nobody can reach it.

**Colour and contrast.** The `--w-*` greys are chosen to clear WCAG AA (4.5:1) on both `--w-surface` and `--w-panel`, and `application.css` records the measurement next to each. The trap is a tint: a status pill or a count badge paints its own 5–13% background, and text that passes on the bare surface can fail on that — three colours here did, one of them *after* the token was lifted. Measure against what is actually behind the glyphs, and prefer `var(--w-muted)` to a fresh hex.

**HTML cards.** A card's bytes live on the `HtmlCard` record; the body carries only an `<action-text-attachment sgid>` (Action Text would scrub the markup on every render, and a 267KB chart shouldn't ride every autosave). They reach a browser at exactly two seams, both of which read the record: `HtmlCard.expand` on the public post, and `Writing::HtmlCardsController#show`, which serves one card bare (`layout: false`, auth-gated like every `Writing::` action) for the editor canvas to frame in `<iframe sandbox="allow-scripts">` — no `allow-same-origin`, and the response asserts `Content-Security-Policy: sandbox allow-scripts; frame-ancestors 'self'`, so a card's script can't read the writer's session from the canvas or from a tab. The canvas fills the sgid into `data-html-card-url-template` on the editor element; it never builds a URL. Everything else — mail, feeds, excerpts — renders the attachment partial's static skeleton (`static_html`, script and style pruned *with* their contents).

**The editor speaks Ghost's `kg-*` dialect.** Lexxy has nodes for prose, images and attachments and nothing else, so the Action Text partials the app overrides emit `kg-*` markup for everything the editor saves: `ImageCardsHelper#kg_image_card` behind `_blob`, `_remote_image` and `_attachment_gallery`, and `_content_attachment` renders its content bare so an opaque card is a fixed point across renders (Action Text's own wrapper grew one `<figure>` per editor load). Going the other way, `app/javascript/lexxy_kg_cards.js` imports `kg-*` figures back into nodes on load: a plain image card (img plus optional figcaption) becomes Lexxy's image node with an editable caption; every other card becomes an opaque `text/html` attachment holding the markup verbatim — galleries, embeds, bookmarks, toggles, callouts, buttons, bare iframes, and image cards the node can't carry (a `srcset`, a `kg-width-*` class, a linked image). `Post::Body.canonicalize` then stores each attachment's content already sanitized, so the first save is the last change. `image_cards_helper.rb` and `lexxy_kg_cards.js` are twins — one paints the figure, the other reads it — and change together, with `test/integration/kg_cards_round_trip_test.rb` holding the server half.

**The render-pass chain.** A body is stored as the editor exported it and decorated on the way out, inside `cache post` in `posts/_post.html.erb`: `HeadingAnchors` (ids), then `CodeHighlighting` (Rouge colours every `<pre>`, class-based tokens, a language label and a copy button), then `HtmlCard.expand` (a card's raw bytes, last so they're never re-parsed). The passes run after sanitization — that is what lets a copy button carry `data-*` — so nothing they add may ever be written back to the body. Rouge is the one gem added for this; it's a format codec, and the editor's Prism variables are mapped onto the same palette so canvas ≈ page.

**Caching.** Public pages use fragment caches keyed on posts plus `fresh_when` ETags. Anything rendered inside `cache post` that lives on another record needs a touch path — e.g. `Tag` touches its posts on rename. If you add such data, add the touch. The render passes are a blind spot: their Ruby is not in the key, so a change to what `CodeHighlighting` or `HeadingAnchors` emits busts nothing by itself. When one changes its output, bump the date in the `Render Pass Dependency Updated` comment inside the `cache` block in `posts/_post.html.erb` — a new template digest busts the fragment and the ETag together.

**Testing.** The default system-test driver is `rack_test` — **no JavaScript executes** (documented in `application_system_test_case.rb`). JS behavior is covered by unit/integration tests + manual dogfood; don't add a browser driver casually, and don't trust a green system test to prove a Stimulus controller works. The one exception is `EditorSystemTestCase` (headless Chrome), for the writing room's keyboard behaviours — the selection bubble and what follows it — which do not exist in the HTML the server sent; those tests live in `test/system/editor/` and nothing else belongs on that class. CI runs on Linux: its clock has ns precision where macOS has µs — another reason time-sensitive tests use floored stamps.
