...
**Keyboard.** A toolbar button's `data-hotkey` is matched against `event.key`, and on a Mac keyboard Option turns the character into a symbol — ⌥2 arrives as `™` — so **no Option chord can ride `data-hotkey`**. The heading chords (⌘⌥2/3/4, ⌘⌥0) are an extension listening on the editor root and matching `event.code` instead, which names the physical key whatever Option did to it — dispatching Lexxy's named `applyHeadingFormat` command with the tag, never one of its private preset commands. WebDriver's synthetic `Alt+2` reports the plain digit, so a passing browser test proves less than it looks like — `test/system/editor/hotkeys_test.rb` also dispatches the real shape (`key: "™"`, `code: "Digit2"`) by hand. Everything else is a Stimulus action on the form or the shell, except ⌘/: Stimulus 3.2 can't filter `/`, so that one reads `event.key` in the controller. The shortcut sheet is a `<dialog>` in ERB, and `test/integration/shortcut_sheet_test.rb` asserts it prints every hotkey the toolbar declares.
