...
**Autosave contract.** Autosave is the *only* save in the editor — there is no Save button — so the `X-Autosave` header buys exactly four answers and nothing else. On an existing record: `204` on success, **bodyless** `422` on validation failure (never `render :edit` — it would repaint the page mid-keystroke), and `200` with a Turbo Stream when the save moved the slug (below). On the first keystroke of a new one, the mint: `201` carrying `Location` (the PATCH target the client adopts verbatim), `X-Post-Id`, and a Turbo Stream body that fills the four empty slots the form left for a saved record — Publish, its popover, Delete, the tag mint. All four shapes live in `Writing::Autosaving`, shared by the posts and pages controllers, because when each spelled them out for itself they drifted. Explicit saves are normal Turbo submits.
